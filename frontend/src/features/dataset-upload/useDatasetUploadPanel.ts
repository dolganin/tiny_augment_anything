...
import { buildPendingImportState, buildPendingProject, buildUploadStatusLabel, isDomAbortError, shouldShowCompactMeta } from './dataset-upload.lib'
import { type DatasetUploadPanelProps } from './dataset-upload.types'

export function useDatasetUploadPanel({
  navigateTo = '/dataset/stats',
  openOnImportComplete = false,
  onProjectChange,
  ...
  const pendingImport = useMemo(() => buildPendingImportState(uploadSession), [uploadSession])
  ...
  const pendingProject = useMemo(
    () =>
      buildPendingProject({
        importErrorMessage: importTaskQuery.data?.error?.message,
        importMessage: importTaskQuery.data?.message,
        importProgress: importTaskQuery.data?.progress,
        importStatus: importTaskQuery.data?.status,
        isUploading,
        pendingImport,
        uploadProgress,
        uploadSession,
  ...
  const uploadStatusLabel = buildUploadStatusLabel({
    importMessage: importTaskQuery.data?.message,
    importProgress,
    isCancelling,
    isImportPending,
  ...
  const showCompactMeta = shouldShowCompactMeta({
    isBusy,
    pendingImport,
    selectedFileName,
    uploadSession,
  ...
  return {
    ...
    handleFileSelect,
    handleResetUpload,
    importMessage: importTaskQuery.data?.message ?? null,
    importProgress,
    isBusy,
    isImportPending,
    isUploading,
